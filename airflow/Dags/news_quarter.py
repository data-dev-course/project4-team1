    ...
    for cnt in range(MAX_RETRIES):
        try:
            ...
            try:
                ...
                print(
                    f"upload_S3 | 장소: {place} | S3 위치: {filepath} | 파일명: {filename} | 성공"
                )
            except Exception as e:
                print(
                    f"upload_S3 | 장소: {place} | S3 위치: {filepath} | 파일명: {filename} | 실패"
                )
...
def async_call_api_task():
    ...
    chunks = [
        area_list[i * chunk_size : (i + 1) * chunk_size] for i in range(num_chunks)
    ] + [last_chunk]
